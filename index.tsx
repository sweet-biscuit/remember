...
const Navbar: React.FC<{ currentView: AppView; setView: (v: AppView) => void }> = ({ currentView, setView }) => {
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md bg-white/90 backdrop-blur-xl border border-white shadow-2xl rounded-[32px] px-2 py-2 flex justify-around items-center z-50">
      {items.map((item) => {
        ...
        return (
          <button key={item.id} onClick={() => setView(item.id)} className={`flex flex-col items-center py-2 px-4 rounded-2xl transition-all ${active ? 'text-indigo-600 scale-110' : 'text-slate-300'}`}>
            ...
            <span className={`text-[10px] font-bold mt-1 ${active ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}`}>{item.label}</span>
          </button>
        );
      })}
    </nav>
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-4 animate-in fade-in duration-500">
      ...
      <div className="flex gap-4 w-full px-2">
        <button onClick={() => handleLevel(-1)} className="flex-1 py-5 bg-white border border-slate-100 rounded-[28px] font-black text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-all">不熟</button>
        <button onClick={() => handleLevel(1)} className="flex-1 py-5 bg-slate-900 text-white rounded-[28px] font-black shadow-xl hover:bg-indigo-600 transition-all">記住了</button>
      </div>
    </div>
  );
};
...
const QuizView: React.FC<{ targetWords: Word[]; allWords: Word[]; onFinish: () => void; onUpdate: (w: Word) => void }> = ({ targetWords, allWords, onFinish, onUpdate }) => {
  ...
  return (
    <div className="space-y-8 py-4 animate-in fade-in duration-500">
      <div className="flex justify-between items-center text-slate-400 font-black text-[10px] uppercase">
      ...
      <div className="bg-white rounded-[40px] p-12 shadow-sm border border-slate-50 text-center relative">
        <button onClick={() => speak(q.word.term)} className="absolute top-6 right-6 text-xl opacity-20 hover:opacity-100">🔊</button>
        <h2 className="text-4xl font-black text-slate-800 tracking-tight">{q.word.term}</h2>
      </div>
      ...
      {selected !== null && (
        <button onClick={() => { if (idx < questions.length - 1) { setIdx(idx + 1); setSelected(null); } else onFinish(); }} className="w-full py-5 bg-slate-900 text-white rounded-[28px] font-black shadow-lg">
          {idx < questions.length - 1 ? '下一題' : '完成結果'}
        </button>
      )}
    </div>
  );
